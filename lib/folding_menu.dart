...
class _FoldingMenuState extends State<FoldingMenu>
  ...
  initiateAnimations(bool folded) {
    _animation =
        (folded ? Tween(begin: 0.0, end: 1.57) : Tween(begin: 1.57, end: 0.0))
            .animate(_curvedAnimation);
    transAnimation =
        (folded ? Tween(begin: 0.0, end: 1.0) : Tween(begin: 1.0, end: 0.0))
            .animate(_curvedAnimation);
  }
    ...
    return Container(
      child: Column(
        children: widgets.map((child) {
          ...
          Color shadowColor =
              widget.shadowColor!.withOpacity(transAnimation.value * 0.35);

          return Transform(
              alignment: Alignment.topCenter,
              transform: Matrix4.translationValues(
                  0,
                  transAnimation.value *
                      (-(positions[child[1]] ?? 0) -
                          (isOdd ? elementHeight : 0)),
                  0),
              child: Transform(
                  alignment:
                      isOdd ? Alignment.bottomCenter : Alignment.topCenter,
